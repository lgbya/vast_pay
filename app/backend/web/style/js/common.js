...
function submitHref(formDoc){
    return submitRefresh(formDoc, 0)
}

function submitRefresh(formDoc, timeNum) {
    if (formDoc.find('.has-error').length){
        return false;
    }
    $.ajax({
        url: formDoc.attr('action'),
        type: 'POST',
        data: formDoc.serialize(),
        success: function (data) {
            console.log(data);
            if (data.code != 0) {
                ShowNotice('提示', data.message, 'error', 1000, false);
            }else{
        ...
        error: function(jqXHR, errMsg) {
            ShowNotice('提示', errMsg, 'error', 1000, false);
        }
    });
    return false;
    // $.ajax({
